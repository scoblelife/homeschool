...
import type { StudentReward, FamilyGoal, CreateFamilyGoal, UpdateFamilyGoal } from '../../types'
...
export async function getStudentRewards(studentId: string, weekStart?: string): Promise<StudentReward[]> {
...
export async function getStudentStarTotals(studentId: string): Promise<{ weeklyTotal: number; allTimeTotal: number }> {
  const db = await getDatabase()
...
export async function createReward(data: Omit<StudentReward, 'id' | 'createdAt'>): Promise<StudentReward> {
  const db = await getDatabase()
  const id = uuid()
  ...
  const row = await db.getFirstAsync('SELECT * FROM student_rewards WHERE id = ?', id)
...
export async function getFamilyGoals(): Promise<FamilyGoal[]> {
  const db = await getDatabase()
  const rows = await db.getAllAsync('SELECT * FROM family_goals ORDER BY created_at DESC') as Record<string, unknown>[]
...
export async function createFamilyGoal(data: CreateFamilyGoal): Promise<FamilyGoal> {
  ...
  const row = await db.getFirstAsync('SELECT * FROM family_goals WHERE id = ?', id)
...
export async function updateFamilyGoal(id: string, data: UpdateFamilyGoal): Promise<FamilyGoal> {
  ...
  const current = await db.getFirstAsync('SELECT * FROM family_goals WHERE id = ?', id)
...
export async function achieveFamilyGoal(id: string): Promise<FamilyGoal> {
  ...
  await db.runAsync('UPDATE family_goals SET achieved_at = ? WHERE id = ?', now, id)
