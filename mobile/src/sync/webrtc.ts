...
export class PeerConnection {
  ...
  private setupEventHandlers(): void {
    // @ts-ignore - react-native-webrtc has different event handler types
    this.pc.onicecandidate = (event: any) => {
      if (event.candidate) {
        ...
        console.log('[WebRTC] ICE gathering complete with', this.collectedCandidates.length, 'candidates')
  ...
  private setupDataChannel(channel: any): void {
    ...
    channel.onmessage = (event: any) => {
      console.log('[WebRTC] Received message:', event.data?.substring?.(0, 100) || event.data)
   ...
   */
  async handleOffer(offer: any, iceCandidates: any[]): Promise<{ answer: any; iceCandidates: any[] }> {
  ...
  private gatherIceCandidates(): Promise<any[]> {
    return new Promise((resolve) => {
      ...
      const timeout = setTimeout(() => {
        console.log('[WebRTC] ICE gathering timed out with', this.collectedCandidates.length, 'candidates')
...
export class WebRTCSwarm {
  ...
  async join(roomId: string): Promise<void> {
    if (!this.signaling) {
      console.log('[WebRTCSwarm] No signaling provider, running in simulation mode')
      return
    }
  ...
  private async handleSignalingMessage(message: SignalingMessage): Promise<void> {
    console.log('[WebRTCSwarm] Received signaling:', message.type, 'from:', message.from)

    if (message.type === 'offer') {
      // Received an offer, create answer
      let peer = this.peers.get(message.from)
      if (!peer) {
        peer = new PeerConnection(this.localId, message.from, this.events)
