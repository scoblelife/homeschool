...
export class FamilyManager {
  ...
  private async saveConfig(): Promise<void> {
    if (this.familyConfig) {
      await AsyncStorage.setItem(FAMILY_CONFIG_KEY, JSON.stringify(this.familyConfig))
    }
    if (this.deviceConfig) {
      await AsyncStorage.setItem(DEVICE_CONFIG_KEY, JSON.stringify(this.deviceConfig))
    }
  }
