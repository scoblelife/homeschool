...
export class EventProjector {
  ...
  private async applyStudentDeleted(event: SyncEvent): Promise<void> {
    const db = await getDatabase()
    await db.runAsync(`DELETE FROM students WHERE id = ?`, toBindValue(event.data.id))
  }
  ...
  private async applyActivityDeleted(event: SyncEvent): Promise<void> {
    const db = await getDatabase()
    await db.runAsync(`DELETE FROM activities WHERE id = ?`, toBindValue(event.data.id))
  }
  ...
  private async applyMilestoneDeleted(event: SyncEvent): Promise<void> {
    const db = await getDatabase()
    await db.runAsync(`DELETE FROM milestones WHERE id = ?`, toBindValue(event.data.id))
  }
  ...
  private async applyFieldTripDeleted(event: SyncEvent): Promise<void> {
    const db = await getDatabase()
    await db.runAsync(`DELETE FROM field_trip_students WHERE field_trip_id = ?`, toBindValue(event.data.id))
    await db.runAsync(`DELETE FROM field_trips WHERE id = ?`, toBindValue(event.data.id))
  }
  ...
  private async applySubjectDeleted(event: SyncEvent): Promise<void> {
    const db = await getDatabase()
    await db.runAsync(`DELETE FROM subjects WHERE id = ?`, toBindValue(event.data.id))
  }
  ...
  private async applyBookDeleted(event: SyncEvent): Promise<void> {
    const db = await getDatabase()
    await db.runAsync(`DELETE FROM books WHERE id = ?`, toBindValue(event.data.id))
  }
}
