...
import type { SignalingProvider, SignalingMessage as WebRTCSignalingMessage } from './webrtc'
...
export class WorkerSignalingProvider implements SignalingProvider {
  ...
  private messageCallback: ((message: WebRTCSignalingMessage) => void) | null = null
  ...
  async send(message: WebRTCSignalingMessage): Promise<void> {
    ...
    await this.client.sendSignal(this.currentRoom, message.to, message as SignalingMessage)
  }

  /**
   * Subscribe to signaling messages for this peer
   */
  subscribe(localId: string, onMessage: (message: WebRTCSignalingMessage) => void): () => void {
  ...
  async joinRoom(roomId: string, localId: string): Promise<string[]> {
    ...
    this.heartbeatInterval = setInterval(async () => {
      if (this.currentRoom && this.localId) {
        try {
          await this.client.heartbeat(this.currentRoom, this.localId, this.pubKey)
        } catch (err) {
  ...
  private startPolling(): void {
    ...
    this.pollInterval = setInterval(async () => {
      ...
      try {
        const messages = await this.client.pollSignals(this.currentRoom, this.localId)
