...
class NotificationsManager {
  ...
  async updateSettings(settings: Partial<NotificationSettings>): Promise<void> {
    if (settings.enabled !== undefined) {
      await AsyncStorage.setItem(STORAGE_KEYS.ENABLED, String(settings.enabled))
    }
...
export function useNotifications() {
  return {
    ...
    scheduleDailyReminder: notifications.scheduleDailyReminder.bind(notifications),
    scheduleStreakWarning: notifications.scheduleStreakWarning.bind(notifications),
    cancelAllNotifications: notifications.cancelAllNotifications.bind(notifications),
