...
export function useStudents() {
  ...
  const updateStudent = useCallback(
    async (id: string, data: UpdateStudent) => {
      const student = await window.api.updateStudent(id, data);
      setStudents(
        useStore.getState().students.map((s) => (s.id === id ? student : s)),
      );
      return student;
    },
    [setStudents],
...
export function useSessions(filters?: {
  ...
  const updateSession = useCallback(
    async (id: string, data: UpdateSession) => {
      const session = await window.api.updateSession(id, data);
      setSessions(
        useStore.getState().sessions.map((s) => (s.id === id ? session : s)),
      );
      return session;
    },
    [setSessions],
...
export function useActivities(filters?: {
  ...
  const updateActivity = useCallback(
    async (id: string, data: UpdateActivity) => {
      const activity = await window.api.updateActivity(id, data);
      setActivities(
        useStore.getState().activities.map((a) => (a.id === id ? activity : a)),
      );
      return activity;
    },
    [setActivities],
...
export function useMilestones(studentId?: string) {
  ...
  const updateMilestone = useCallback(
    async (id: string, data: UpdateMilestone) => {
      const milestone = await window.api.updateMilestone(id, data);
      setMilestones(
        useStore
          .getState()
          .milestones.map((m) => (m.id === id ? milestone : m)),
      );
      return milestone;
    },
    [setMilestones],
  ...
  const initializeMilestones = useCallback(
    async (studentId: string, gradeLevel: GradeLevel) => {
      const newMilestones = await window.api.initializeStudentMilestones(
        studentId,
        gradeLevel,
      );
