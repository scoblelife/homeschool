...
export default function RecurringActivities({
  onActivityCreated,
}: RecurringActivitiesProps): JSX.Element {
  const {
    students,
    subjects,
    selectedStudentId,
  ...
  const [formData, setFormData] = useState<
    Omit<CreateRecurringActivity, "isActive">
  >({
    studentId: "",
    subjectId: "",
    title: "",
  ...
  const resetForm = (): void => {
    setFormData({
      studentId:
        selectedStudentId || (students.length === 1 ? students[0].id : ""),
      subjectId: "",
      title: "",
      activityType: "worksheet",
      durationMinutes: 30,
  ...
  if (dueActivities.length === 0 && allRecurring.length === 0) {
    return (
      <Card className="mb-6 bg-gradient-to-r from-student-blue-50 to-student-fuchsia-50 border-student-blue-100">
  ...
  return (
    <>
      ...
      {dueActivities.length > 0 && (
        <Card className="mb-6 bg-gradient-to-r from-amber-50 to-yellow-50 border-status-warningLight">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button
              ...
              className="text-status-warningDark hover:text-status-warningDark"
            >
              Manage Schedules
            </Button>
          </div>
          ...
          <div className="space-y-3">
            {dueActivities.map((activity) => {
              ...
              return (
                <div
                  key={activity.id}
                  className="flex items-center gap-3 p-3 bg-white rounded-lg border border-status-warningLight"
                >
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      className="bg-status-warning hover:bg-status-warningDark"
                    >
                      Done!
                    </Button>
                  </div>
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            ...
            >
              <Dialog.Panel className="w-full max-w-lg transform overflow-hidden rounded-2xl bg-white shadow-xl transition-all">
                <div className="bg-student-blue-600 px-6 py-4">
                  ...
                  {activities.length === 0 ? (
                    ...
                    <div className="space-y-3 max-h-96 overflow-y-auto">
                      {activities.map((activity) => {
                        ...
                        const subject = getSubjectById(activity.subjectId);
                        const typeInfo = getTypeInfo(activity.activityType);
                        const patternLabel = recurrenceOptions.find(
                          (r) => r.value === activity.recurrencePattern,
                        )?.label;
                        ...
                        return (
                          ...
                          >
                            ...
                            <Button
                              ...
                              className="text-status-error hover:bg-status-errorLight"
                            >
                              Delete
                            </Button>
                          </div>
            ...
            >
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white shadow-xl transition-all">
                <div className="bg-student-blue-600 px-6 py-4">
                  ...
                  <p className="text-student-blue-200 text-sm mt-1">
