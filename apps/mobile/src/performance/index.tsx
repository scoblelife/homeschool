...
class MobilePerformanceMonitor {
    ...
    if (mark) {
      ...
      InteractionManager.runAfterInteractions(() => {
        ...
        if (mark.duration > 2000) {
          console.warn(
            `[Performance] Cold start exceeded 2s target: ${mark.duration}ms`,
          );
        }
      });
    }
  ...
  markScreenReady(screenName: string): void {
    ...
    if (mark) {
      InteractionManager.runAfterInteractions(() => {
        ...
        console.log(
          `[Performance] Screen "${screenName}" ready: ${mark.duration}ms`,
        );

        // Warn for slow screens (>500ms)
        if (mark.duration > 500) {
          console.warn(
            `[Performance] Slow screen "${screenName}": ${mark.duration}ms`,
          );
        }
      });
    }
  ...
  markDbQueryEnd(id: string): void {
    ...
    if (mark) {
      ...
      if (mark.duration > 100) {
        console.warn(
          `[Performance] Slow query "${mark.name}": ${mark.duration}ms`,
        );
      }
  ...
  getSummary(): {
    ...
    for (const [screen, times] of Object.entries(
      this.metrics.screenLoadTimes,
    )) {
      avgScreenLoadTimes[screen] =
