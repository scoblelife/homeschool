/**
 * Mobile Cloudflare Worker Signaling Client
 *
...
export class SignalingClient {
  ...
  async postOffer(topic: string, offer: JoinOffer): Promise<void> {
    const res = await fetch(
      `${this.workerUrl}/offer/${encodeURIComponent(topic)}`,
      {
        method: "POST",
  ...
  async postAnswer(topic: string, answer: JoinAnswer): Promise<void> {
    const res = await fetch(
      `${this.workerUrl}/answer/${encodeURIComponent(topic)}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async getOnlinePeers(familyId: string): Promise<OnlinePeer[]> {
    const res = await fetch(
      `${this.workerUrl}/presence/${encodeURIComponent(familyId)}`,
    );
    if (!res.ok) {
