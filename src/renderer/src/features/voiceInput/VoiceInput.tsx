...
const activityTypeKeywords: Record<string, ActivityType> = {
  ...
  'book': 'reading',
  'print': 'writing_print',
  'printing': 'writing_print',
  'cursive': 'writing_cursive',
  'writing': 'writing_print',
  'wrote': 'writing_print',
  'hands on': 'hands_on',
  'hands-on': 'hands_on',
  'craft': 'hands_on',
...
export default function VoiceInput({ onActivityCreated }: VoiceInputProps): JSX.Element | null {
  ...
  const parseTranscript = useCallback((text: string): ParsedActivity => {
    ...
    const minuteMatch = lowerText.match(/(\d+)\s*(?:minute|min|minutes|mins)/i)
    ...
    title = title.replace(/^(log|add|record|create|new activity|activity)\s*/i, '')
  ...
  return (
    <>
      ...
      <button
        onClick={handleOpen}
        className="fixed bottom-6 right-24 w-14 h-14 bg-green-600 hover:bg-green-700 text-white rounded-full shadow-lg hover:shadow-xl transition-all flex items-center justify-center text-2xl z-40"
        title="Voice Input"
      >
        🎤
      </button>
                  ...
                  <div className="p-6">
                      ...
                      <div className="mb-4 p-4 bg-green-50 rounded-lg border border-green-200">
                        ...
                        <div className="space-y-2 text-sm">
                          <div className="flex justify-between">
                            ...
                            <span className={parsedActivity.studentName ? 'text-green-700 font-medium' : 'text-red-500'}>
                              {parsedActivity.studentName || 'Not detected'}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Subject:</span>
                            <span className={parsedActivity.subjectName ? 'text-green-700 font-medium' : 'text-red-500'}>
                              {parsedActivity.subjectName || 'Not detected'}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Type:</span>
                            <span className={parsedActivity.activityType ? 'text-green-700 font-medium' : 'text-gray-500'}>
                              {parsedActivity.activityType ? activityTypeLabels[parsedActivity.activityType] : 'Worksheet (default)'}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Duration:</span>
                            <span className={parsedActivity.durationMinutes ? 'text-green-700 font-medium' : 'text-gray-500'}>
                              {parsedActivity.durationMinutes ? `${parsedActivity.durationMinutes} minutes` : 'Not specified'}
                            </span>
                          </div>
                          <div className="pt-2 border-t border-green-200">
                            <span className="text-gray-600">Title: </span>
                            <span className="text-green-700 font-medium">{parsedActivity.title}</span>
                          </div>
                        </div>
                            ...
                            <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                              <div
                                className={`h-full transition-all ${
                                  parsedActivity.confidence >= 0.75 ? 'bg-green-500' :
                                  parsedActivity.confidence >= 0.5 ? 'bg-yellow-500' : 'bg-red-500'
                                }`}
                                style={{ width: `${parsedActivity.confidence * 100}%` }}
                              />
                            </div>
                    ...
                    <div className="flex gap-3">
                      ...
                      <button
                        ...
                        disabled={!parsedActivity || !parsedActivity.studentId || !parsedActivity.subjectId || isProcessing}
