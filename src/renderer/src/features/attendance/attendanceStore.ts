...
import type { AttendanceRecord, AttendanceStatus, Student } from '../../../../shared/types'
...
interface AttendanceState {
  ...
  loadStats: (studentId: string, startDate: string, endDate: string) => Promise<void>
  setAttendance: (studentId: string, date: string, status: AttendanceStatus, notes?: string) => Promise<void>
...
export const useAttendanceStore = create<AttendanceState>((set, get) => ({
  ...
  loadAttendance: async (studentId, startDate, endDate) => {
    set({ isLoading: true })
    try {
      const records = await window.api.getAttendanceRecords(studentId, startDate, endDate)
      set((state) => ({
        records: {
          ...state.records,
          [studentId]: records
  ...
  loadStats: async (studentId, startDate, endDate) => {
    try {
      const stats = await window.api.getAttendanceStats(studentId, startDate, endDate)
      set((state) => ({
        stats: {
          ...state.stats,
          [studentId]: stats
...
export function getSchoolYearRange(date: Date): { startDate: string; endDate: string } {
  const year = date.getFullYear()
  const month = date.getMonth()
