...
import type { CoopGroup, CreateCoopGroup, CoopMember, CreateCoopMember } from '../../../../shared/types'
...
export function CoopGroupList({ onSelectGroup }: CoopGroupListProps) {
  ...
  const handleJoinGroup = async () => {
    ...
    try {
      const group = await window.api.getCoopGroupByInviteCode(joinCode.trim().toUpperCase())
      if (!group) {
      ...
      const existingMember = members.find(m => m.familyName.toLowerCase() === familyName.trim().toLowerCase())
      if (existingMember) {
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Your Co-op Groups</h2>
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Join or create groups to coordinate activities with other homeschool families.
          </p>
        </div>
