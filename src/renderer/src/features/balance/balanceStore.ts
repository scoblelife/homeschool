...
interface BalanceState {
  ...
  // Actions
  setSubjectTarget: (studentId: string, subjectId: string, targetMinutesPerWeek: number) => void
...
export const useBalanceStore = create<BalanceState>()(
  persist(
    (set, get) => ({
      ...
      setSubjectTarget: (studentId: string, subjectId: string, targetMinutesPerWeek: number) => {
        const targets = get().studentTargets
        const studentData = targets[studentId] || []
        const existingIndex = studentData.findIndex((t) => t.subjectId === subjectId)
