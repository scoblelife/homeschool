...
interface HourTrackingState {
  ...
  // Actions
  loadSummary: (studentId: string, schoolYear: string, stateRequirements?: StateHourRequirements) => Promise<void>
  getSchoolYearDateRange: (schoolYear: string) => { startDate: string; endDate: string }
...
export const useHourTrackingStore = create<HourTrackingState>((set, get) => ({
  ...
  loadSummary: async (studentId: string, schoolYear: string, stateRequirements?: StateHourRequirements) => {
    set({ isLoading: true, error: null })

    try {
      const { startDate, endDate } = get().getSchoolYearDateRange(schoolYear)
      const activitySummary = await window.api.getActivitySummary(studentId, startDate, endDate)
      ...
      const bySubject: HourTarget[] = activitySummary.map(s => {
        ...
        const targetMin = subjectTarget ? subjectTarget * 60 : totalMinutes / activitySummary.length
      ...
      const minutesPerDayNeeded = targetMinutes && daysRemaining > 0
        ? Math.max(0, Math.ceil((targetMinutes - totalMinutes) / daysRemaining))
        : null
      ...
      set({
        error: err instanceof Error ? err.message : 'Failed to load hour summary',
        isLoading: false
      })
    }
  }
