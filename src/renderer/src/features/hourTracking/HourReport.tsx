...
import { useHourTrackingStore, type StateHourRequirements } from './hourTrackingStore'
...
export function HourReport({ studentId }: Props) {
  ...
  const { summary, isLoading, loadSummary, getSchoolYearDateRange } = useHourTrackingStore()
  ...
  useEffect(() => {
    if (selectedStudentId && schoolYear) {
      const stateData = (stateRequirements.states as Record<string, { requiredHoursPerYear: number | null }>)[selectedState]
      const requirements: StateHourRequirements | undefined = stateData?.requiredHoursPerYear
  ...
  const stateOptions = useMemo(() => {
    return Object.entries(stateRequirements.states as Record<string, { name: string; requiredHoursPerYear: number | null }>)
      .map(([code, data]) => ({
        code,
        name: data.name,
          ...
          >
            {stateOptions.map(state => (
              <option key={state.code} value={state.code}>
                {state.name} {state.hasHourRequirement ? `(${state.hours} hrs)` : '(No hour req)'}
              </option>
            ))}
          </select>
        </div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            {/* Target Hours */}
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
              <p className="text-sm text-gray-500 dark:text-gray-400">Target Hours</p>
              <p className="text-3xl font-bold text-gray-900 dark:text-white">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
              ...
              <p className={`text-3xl font-bold ${
                ...
                {summary.percentComplete !== null ? `${summary.percentComplete}%` : 'N/A'}
              </p>
              {summary.percentComplete !== null && summary.percentComplete < 100 && (
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  {Math.round((summary.targetMinutes! - summary.totalMinutes) / 60)} hrs remaining
                </p>
              )}
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
              ...
              {summary.minutesPerDayNeeded !== null && summary.minutesPerDayNeeded > 0 && (
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  {Math.round(summary.minutesPerDayNeeded / 60 * 10) / 10} hrs/day needed
                </p>
              )}
            </div>
          </div>
          ...
          {summary.targetMinutes !== null && (
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
              <div className="flex justify-between mb-2">
                ...
                <span className={`text-sm font-semibold ${getProgressTextColor(summary.percentComplete!)}`}>
              ...
              {summary.percentComplete !== null && summary.percentComplete < 50 && (
                <div className="mt-3 p-3 bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                  <p className="text-sm text-yellow-700 dark:text-yellow-300">
                    <AlertIcon /> You're behind schedule. Consider logging more instructional hours to meet the {summary.targetHours} hour requirement.
                  </p>
                </div>
              )}
            </div>
