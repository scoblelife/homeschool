...
export function PrintableHourReport({
  studentId,
  schoolYear,
  selectedState,
}: Props) {
  ...
  useEffect(() => {
    if (studentId && schoolYear) {
      setIsLoading(true);
      window.api
        .getActivitySummary(studentId, dateRange.startDate, dateRange.endDate)
    ...
    <div>
      ...
      <div ref={printRef} className="bg-white p-8 print:p-0">
        ...
        <div className="text-center mb-8 border-b pb-6">
          ...
          <p className="text-gray-600 mt-2">School Year: {schoolYear}</p>
        </div>
        ...
        <div className="mb-8">
          <h2 className="text-lg font-semibold text-gray-900 mb-3">
            State Requirements ({stateData?.name})
          </h2>
          <table className="w-full">
            <tbody>
              <tr>
          ...
          <table className="w-full border border-gray-300">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="py-2 px-4 text-right border-b border-gray-300">
                  Activities
                </th>
                <th className="py-2 px-4 text-right border-b border-gray-300">
                  Minutes
            ...
            <tbody>
              {activitySummary.map((subject) => (
                <tr key={subject.subjectId}>
                  ...
                  <td className="py-2 px-4 text-right border-b border-gray-200">
                    {totalMinutes > 0
                      ? Math.round((subject.totalMinutes / totalMinutes) * 100)
                      : 0}
                    %
                  </td>
                </tr>
              ...
              <tr className="font-semibold bg-gray-50">
                ...
                <td className="py-2 px-4 text-right border-t-2 border-gray-400">
                  {totalActivities}
                </td>
                <td className="py-2 px-4 text-right border-t-2 border-gray-400">
                  {totalMinutes}
            ...
            <div
              className={`p-4 rounded-lg ${
                totalHours >= stateData.requiredHoursPerYear
                  ...
                  : "bg-status-warningLight border border-status-warning/20"
              }`}
            >
              {totalHours >= stateData.requiredHoursPerYear ? (
                <p className="text-status-successDark">
                  <CheckIcon /> This student has met the{" "}
                  {stateData.requiredHoursPerYear} hour requirement for{" "}
                  {stateData.name} with {totalHours} total instructional hours
                  logged.
                </p>
              ) : (
                <p className="text-status-warningDark">
                  <AlertIcon /> This student has logged {totalHours} of the
                  required {stateData.requiredHoursPerYear} hours.{" "}
                  {Math.round(stateData.requiredHoursPerYear - totalHours)}{" "}
                  hours remaining.
                </p>
              )}
            </div>
        ...
        <div className="mt-12 pt-8 border-t">
          <p className="text-sm text-gray-600 mb-6">
            I certify that the above information is accurate and represents the
            instructional hours provided to this student during the specified
            school year.
          </p>
        ...
        <div className="mt-12 text-center text-xs text-gray-400">
          <p>
            Generated by Homeschool App on {new Date().toLocaleDateString()}
          </p>
        </div>
      </div>
    </div>
