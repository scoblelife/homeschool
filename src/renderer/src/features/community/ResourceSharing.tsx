...
import type { SharedResource, CreateSharedResource, SharedResourceType, ResourceRating, CoopGroup, CoopMember } from '../../../../shared/types'

type ExtendedSharedResource = SharedResource & { groupName: string; sharedByName: string }

const resourceTypeConfig: Record<SharedResourceType, { icon: string; label: string; color: string; bg: string }> = {
  link: { icon: '🔗', label: 'Link', color: 'text-blue-600', bg: 'bg-blue-100' },
  template: { icon: '📋', label: 'Template', color: 'text-purple-600', bg: 'bg-purple-100' },
  curriculum: { icon: '📚', label: 'Curriculum', color: 'text-green-600', bg: 'bg-green-100' },
  book: { icon: '📖', label: 'Book', color: 'text-amber-600', bg: 'bg-amber-100' },
  other: { icon: '📁', label: 'Other', color: 'text-gray-600', bg: 'bg-gray-100' }
}
...
export function ResourceSharing() {
  ...
  const [selectedResource, setSelectedResource] = useState<ExtendedSharedResource | null>(null)

  // Filters
  const [searchQuery, setSearchQuery] = useState('')
  const [typeFilter, setTypeFilter] = useState<SharedResourceType | 'all'>('all')
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        ...
        <select
          value={typeFilter}
          onChange={(e) => setTypeFilter(e.target.value as SharedResourceType | 'all')}
      ...
      {filteredResources.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 dark:bg-gray-800 rounded-lg">
          ...
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
            {resources.length === 0 ? 'No shared resources yet' : 'No matching resources'}
          </h3>
...
function ResourceCard({ resource, onClick }: { resource: ExtendedSharedResource; onClick: () => void }) {
  const config = resourceTypeConfig[resource.resourceType] || resourceTypeConfig.other
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-5 gap-2">
                {(Object.entries(resourceTypeConfig) as [SharedResourceType, typeof resourceTypeConfig.link][]).map(
                  ([type, config]) => (
                    <button
                      key={type}
                      type="button"
...
function ResourceDetailModal({
  ...
  const [ratings, setRatings] = useState<(ResourceRating & { memberName: string })[]>([])
  const [loadingRatings, setLoadingRatings] = useState(false)

  useEffect(() => {
    if (resource) {
      setLoadingRatings(true)
      window.api.getResourceRatings(resource.id)
        .then(setRatings)
        .catch(console.error)
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <Dialog.Panel className="mx-auto max-w-lg w-full bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6">
          ...
          <div className="space-y-2 mb-4">
            ...
            {resource.subject && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-600 dark:text-gray-400">{resource.subject}</span>
              </div>
            )}
            {resource.gradeLevel && (
              <div className="flex items-center gap-2">
                <AcademicCapIcon className="w-4 h-4 text-gray-400" />
                <span className="text-gray-600 dark:text-gray-400">{resource.gradeLevel}</span>
              </div>
            )}
          </div>
            ...
            {resource.ratingCount > 0 ? (
              <>
                ...
                <span className="text-gray-600 dark:text-gray-400">
                  {resource.averageRating.toFixed(1)} ({resource.ratingCount} {resource.ratingCount === 1 ? 'rating' : 'ratings'})
                </span>
              </>
            ) : (
          ...
          {loadingRatings ? (
            ...
            <div className="mb-4">
              <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Reviews</h4>
              <div className="space-y-2 max-h-32 overflow-y-auto">
                {ratings.filter(r => r.review).map(rating => (
                  <div key={rating.id} className="text-sm p-2 bg-gray-50 dark:bg-gray-700 rounded">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-gray-500">{rating.memberName}</span>
                    </div>
          ...
          <div className="text-xs text-gray-400 mb-4">
            Shared by {resource.sharedByName} on {format(parseISO(resource.createdAt), 'MMM d, yyyy')}
          </div>
