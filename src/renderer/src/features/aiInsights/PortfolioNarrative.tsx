...
export function PortfolioNarrative({
  ...
  const loadActivityData = async () => {
    try {
      ...
      const data = Array.from(subjectMap.values()).sort((a, b) => b.minutes - a.minutes)
      setSubjectData(data)
      setTotalActivities(data.reduce((sum, s) => sum + s.activities, 0))
      setTotalHours(Math.round(data.reduce((sum, s) => sum + s.minutes, 0) / 60))
    } catch (err) {
...
The narrative should read as a cohesive educational summary suitable for inclusion in an official portfolio.`
      ...
      const result = await window.api.aiComplete(prompt, {
        maxTokens: 800,
        temperature: 0.6,
        systemPrompt: 'You are an experienced homeschool education evaluator writing professional portfolio narratives.',
        useCache: true,
      })
      ...
      setLocalError(err instanceof Error ? err.message : 'Failed to generate narrative')
    } finally {
      setIsLoading(false)
    }
  }
    ...
    return (
      <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
        <p className="text-sm text-gray-500">
          AI narrative generation requires an API key. Configure in Settings to enable this feature.
        </p>
      </div>
    )
  }
      ...
      {narrative && (
        <div className="border border-gray-200 rounded-lg overflow-hidden">
          <div className="bg-gray-50 px-4 py-2 border-b border-gray-200 flex items-center justify-between">
            <span className="text-sm font-medium text-gray-700">Generated Narrative</span>
          ...
          <div className="bg-gray-50 px-4 py-2 border-t border-gray-200">
            <p className="text-xs text-gray-500">
              Generated by AI. Review and edit as needed before including in your portfolio.
            </p>
          </div>
        </div>
      )}
