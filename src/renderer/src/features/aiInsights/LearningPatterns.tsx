  ...
  const analyzePatterns = useCallback(async () => {
    ...
    try {
      ...
      const [sessions, activities, summary] = await Promise.all([
        window.api.getSessions({ studentId, startDate: sixtyDaysAgo, endDate: today }),
        window.api.getActivities({ studentId, startDate: sixtyDaysAgo, endDate: today }),
      ...
      const sessionData: SessionData[] = sessions
        ...
        .map((s) => {
          const subject = summary.find((sum) => sum.subjectId === s.subjectId)?.subjectName || 'Unknown'
          const hour = parseInt(s.startTime!.split(':')[0])
          // Calculate duration from start and end time
          const [startH, startM] = s.startTime!.split(':').map(Number)
          const [endH, endM] = s.endTime!.split(':').map(Number)
          const duration = Math.max(0, (endH * 60 + endM) - (startH * 60 + startM)) || 30
      ...
      const timeAnalysis: Array<{ period: string; avgDuration: number; count: number }> = []
      ...
      const subjectBreakdown = summary
        .map((s) => `- ${s.subjectName}: ${s.totalActivities} activities, ${Math.round(s.totalMinutes / 60)} hours`)
        .join('\n')

      const timeBreakdown = timeAnalysis
        .map((t) => `- ${t.period}: ${t.count} sessions, avg ${t.avgDuration} min`)
        .join('\n')

      const activityTypes = new Map<string, number>()
      activities.forEach((a) => {
        activityTypes.set(a.activityType, (activityTypes.get(a.activityType) || 0) + 1)
      })
...
Be specific with numbers. Make insights actionable. Return only valid JSON.`
      ...
      const result = await window.api.aiComplete(prompt, {
        maxTokens: 600,
        temperature: 0.5,
        systemPrompt: 'You are a learning analytics expert providing data-driven insights about student learning patterns.',
        useCache: true,
      })
      ...
      setLocalError(err instanceof Error ? err.message : 'Failed to analyze patterns')
    } finally {
      setIsLoading(false)
    }
  ...
  return (
    <div className="card bg-gradient-to-r from-blue-50 to-cyan-50 border-blue-100">
      <button
        onClick={() => patterns.length > 0 ? setIsExpanded(!isExpanded) : analyzePatterns()}
      ...
      {isExpanded && patterns.length > 0 && (
        <div className="mt-4 space-y-3">
          {patterns.map((pattern, index) => (
            <div key={index} className="bg-white/60 rounded-lg p-4">
              <div className="flex items-start gap-3">
                <span className="text-2xl">{categoryIcons[pattern.category] || '💡'}</span>
                <div className="flex-1">
                  ...
                  <h4 className="font-medium text-gray-900">{pattern.insight}</h4>
          ...
          <div className="flex items-center justify-between text-xs text-blue-600 pt-2">
            <span>
              Analyzed by AI{lastAnalyzed ? ` • ${format(lastAnalyzed, 'h:mm a')}` : ''}
            </span>
            <button
              onClick={(e) => {
                e.stopPropagation()
