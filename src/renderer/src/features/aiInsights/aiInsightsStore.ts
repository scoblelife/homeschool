...
interface AIInsightsState {
  ...
  generateWeeklySummary: (
    ...
    weekData: {
      ...
      totalMinutes: number
      activeDays: number
      subjectBreakdown: Array<{ name: string; activities: number; minutes: number }>
...
export const useAIInsightsStore = create<AIInsightsState>((set, get) => ({
  ...
  initialize: async () => {
    try {
      ...
      set({ isInitialized: true, isAvailable: false, error: 'Failed to initialize AI' })
    }
  },
  ...
  generateWeeklySummary: async (studentId, studentName, gradeLevel, weekStart, weekData) => {
    // Check cache first
    ...
    try {
      ...
      const subjectList = weekData.subjectBreakdown
        .map((s) => `  - ${s.name}: ${s.activities} activities, ${Math.round(s.minutes)} minutes`)
        .join('\n')

      const activityChange = weekData.totalActivities - weekData.previousWeekActivities
      const minutesChange = weekData.totalMinutes - weekData.previousWeekMinutes
...
Keep the tone warm, specific, and parent-friendly. Avoid generic phrases. Focus on the data provided.`
      ...
      const result = await window.api.aiComplete(prompt, {
        maxTokens: 500,
        temperature: 0.7,
        systemPrompt: 'You are a supportive homeschool education assistant helping parents track and celebrate their children\'s learning progress.',
        useCache: true,
      })
