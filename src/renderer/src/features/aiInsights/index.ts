/**
 * AI Insights Feature
 *
 * Provides AI-powered analysis and suggestions for homeschool activities.
 */

export { AIWeeklySummary } from './AIWeeklySummary'
export { ActivitySuggestions } from './ActivitySuggestions'
export { PortfolioNarrative } from './PortfolioNarrative'
export { LearningPatterns } from './LearningPatterns'
export { CompliancePrediction } from './CompliancePrediction'
export { SubjectSuggestions, useSmartCategorization } from './SmartCategorization'
export { ChatLogger } from './ChatLogger'
export { useAIInsightsStore } from './aiInsightsStore'
