...
export function useSmartCategorization(subjects: Subject[]): {
  ...
  const getSuggestions = useCallback(async (description: string): Promise<SubjectSuggestion[]> => {
 ...
 */
function keywordMatch(description: string, subjects: Subject[]): SubjectSuggestion[] {
  const lower = description.toLowerCase()
  const suggestions: SubjectSuggestion[] = []

  const keywords: Record<string, string[]> = {
    'math': ['math', 'count', 'number', 'add', 'subtract', 'multiply', 'divide', 'lego', 'blocks', 'measure', 'bake', 'cook', 'recipe', 'fraction', 'geometry', 'shape'],
    'language arts': ['read', 'write', 'book', 'story', 'spell', 'word', 'letter', 'journal', 'poem', 'essay', 'vocabulary', 'grammar'],
    'science': ['science', 'experiment', 'nature', 'animal', 'plant', 'weather', 'space', 'chemistry', 'biology', 'physics', 'observe', 'lab'],
    'social studies': ['history', 'geography', 'map', 'country', 'culture', 'community', 'government', 'president', 'museum', 'field trip'],
    'art': ['art', 'draw', 'paint', 'color', 'craft', 'create', 'design', 'sculpture', 'clay'],
    'music': ['music', 'sing', 'song', 'instrument', 'piano', 'guitar', 'drum', 'dance', 'rhythm'],
    'physical education': ['exercise', 'sport', 'run', 'swim', 'bike', 'soccer', 'basketball', 'gym', 'outside', 'play', 'walk', 'hike'],
  }
...
export function SubjectSuggestions({
  ...
  if (!hasSearched && description.length >= 5 && suggestions.length === 0 && !isLoading) {
