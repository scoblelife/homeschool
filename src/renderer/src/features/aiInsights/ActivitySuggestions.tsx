...
export function ActivitySuggestions({
  ...
  const { isInitialized, isAvailable, isGenerating, error, initialize } = useAIInsightsStore()
  ...
  const loadSubjectData = useCallback(async () => {
    ...
    try {
      const summary = await window.api.getActivitySummary(studentId, thirtyDaysAgo, today)
  ...
  const generateSuggestions = async () => {
    ...
    try {
      ...
      const avgActivities = subjectData.reduce((sum, s) => sum + s.totalActivities, 0) / subjectData.length
      const underrepresented = subjectData.filter(
        (s) => s.totalActivities === 0 || s.totalActivities < avgActivities * 0.5
      ).slice(0, 3)
      ...
      if (underrepresented.length === 0) {
        setSuggestions([{
          ...
          ideas: ['Keep up the variety!', 'Try a new activity type in your favorite subject']
        }])
        setIsExpanded(true)
      ...
      const subjectList = underrepresented
        .map((s) => `- ${s.subjectName}: ${s.totalActivities} activities in past 30 days`)
        .join('\n')
...
Only return the JSON array, no other text.`
      ...
      const result = await window.api.aiComplete(prompt, {
        maxTokens: 500,
        temperature: 0.7,
        systemPrompt: 'You are a creative homeschool curriculum assistant. Return only valid JSON.',
        useCache: true,
      })
      ...
      try {
        ...
        setSuggestions(underrepresented.map((s) => ({
      ...
      setLocalError(err instanceof Error ? err.message : 'Failed to generate suggestions')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const needsAttention = subjectData.filter((s) => s.totalActivities === 0).length

  return (
    <div className="card bg-gradient-to-r from-emerald-50 to-teal-50 border-emerald-100">
      <button
        onClick={() => suggestions.length > 0 ? setIsExpanded(!isExpanded) : generateSuggestions()}
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-gray-900">AI Activity Suggestions</h3>
          ...
          <div className="flex items-center gap-2">
            {(isLoading || isGenerating) ? (
              ...
              <span className="text-sm text-emerald-600 font-medium">Generate</span>
            )}
          </div>
        </div>
      </button>
      ...
      {isExpanded && suggestions.length > 0 && (
        <div className="mt-4 space-y-4">
            ...
            <div key={index} className="bg-white/60 rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h4 className="font-medium text-gray-900">{suggestion.subject}</h4>
          ...
          <div className="flex items-center justify-between text-xs text-emerald-600 pt-2">
            <span>
              Generated by AI{lastGenerated ? ` • ${format(lastGenerated, 'h:mm a')}` : ''}
            </span>
            <button
              onClick={(e) => {
                e.stopPropagation()
