...
export function ActivityAttachments({ activityId, readOnly = false }: Props) {
  ...
  if (loading) {
    return (
      <div className="text-sm text-neutral-textSecondary py-2">
        Loading attachments...
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        {!readOnly && (
          /* eslint-disable-next-line design-system/require-design-system-components -- compact inline add button */
          <button
            onClick={handleAdd}
            disabled={adding}
            className="inline-flex items-center px-2 py-1 text-xs font-medium text-brand-primary hover:text-brand-primaryDark disabled:opacity-50"
          >
            <svg
      ...
      {attachments.length > 0 ? (
        <div className="grid grid-cols-4 gap-2">
          {attachments.map((attachment) => (
            <div
              key={attachment.id}
              className="relative group aspect-square rounded-lg overflow-hidden bg-neutral-backgroundSecondary cursor-pointer hover:ring-2 hover:ring-brand-primary"
            ...
            >
              ...
              />
              {!readOnly && (
                /* eslint-disable-next-line design-system/require-design-system-components -- overlay delete button on image */
                <button
                  onClick={(e) => {
                    e.stopPropagation();
        ...
        <div className="text-sm text-neutral-textTertiary py-4 text-center border-2 border-dashed border-neutral-border rounded-lg">
          {readOnly
            ? "No photos"
                  ...
                  {selectedAttachment && (
                    <>
                      ...
                      <div className="absolute top-2 right-2 flex gap-2">
                        {/* eslint-disable-next-line design-system/require-design-system-components -- overlay button on image viewer */}
                        <button
                          onClick={() =>
                            handleOpenFile(selectedAttachment.filePath)
                          }
                          className="p-2 bg-white/90 rounded-full hover:bg-white"
                          title="Open in default app"
                        >
                          <svg
                            className="w-5 h-5 text-neutral-text"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
                        ...
                        {/* eslint-disable-next-line design-system/require-design-system-components -- overlay close button on image viewer */}
                        <button
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-black/50 text-white text-sm p-2 rounded-b-lg">
                        ...
                        {selectedAttachment.width &&
                          selectedAttachment.height && (
                            <span className="opacity-70 ml-2">
                              ({selectedAttachment.width} x{" "}
                              {selectedAttachment.height})
                            </span>
                          )}
                      </div>
  ...
  return (
    <div className="flex items-center gap-1 mt-1">
      {attachments.slice(0, 3).map((attachment) => (
        <div
          key={attachment.id}
          className="w-8 h-8 rounded overflow-hidden bg-neutral-backgroundSecondary"
        >
          <img
      ...
      {attachments.length > 3 && (
        <div className="w-8 h-8 rounded bg-neutral-border flex items-center justify-center text-xs text-neutral-textSecondary">
          +{attachments.length - 3}
        </div>
      )}
    </div>
