...
import type { Activity, FieldTripActivity, FieldTrip } from '../../../../shared/types'
...
export function LinkedActivities({ fieldTrip, onLink, onUnlink }: LinkedActivitiesProps) {
  ...
  const loadLinkedActivities = useCallback(async () => {
    try {
      ...
      if (activityIds.length > 0) {
        ...
        const linked = allActivities.filter((a: Activity) => activityIds.includes(a.id))
  ...
  return (
    <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
      ...
      {/* Link Activity Modal */}
      <Dialog open={isLinkModalOpen} onClose={() => setIsLinkModalOpen(false)} className="relative z-50">
          ...
          <Dialog.Panel className="mx-auto max-w-md w-full bg-white dark:bg-gray-800 rounded-xl shadow-xl">
            <div className="p-6">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                Activities from {format(subDays(parseISO(fieldTrip.date), 7), 'MMM d')} to{' '}
              ...
              {availableActivities.length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-gray-400 italic py-4 text-center">
                  No activities available to link. Log activities around the field trip date first.
                </p>
              ) : (
...
export function LinkedCountBadge({ fieldTripId }: LinkedCountBadgeProps) {
  ...
  useEffect(() => {
    window.api.getLinkedActivities(fieldTripId).then((links: FieldTripActivity[]) => {
