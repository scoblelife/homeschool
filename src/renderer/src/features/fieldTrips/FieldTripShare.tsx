...
export function FieldTripShare({ trip, students, subjects, isOpen, onClose }: FieldTripShareProps) {
  ...
  const textSummary = useMemo(() => {
    ...
    if (trip.startTime) {
      lines.push(`Time: ${trip.startTime}${trip.endTime ? ` - ${trip.endTime}` : ''}`)
    }
  ...
  const icsContent = useMemo(() => {
    ...
    const description = [
      trip.description,
      trip.learningOutcomes ? `Learning Outcomes: ${trip.learningOutcomes}` : null,
