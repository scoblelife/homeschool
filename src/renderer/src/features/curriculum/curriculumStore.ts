...
interface CurriculumState {
  ...
  loadReport: (studentId: string, gradeLevel: GradeLevel, startDate?: string, endDate?: string) => Promise<void>
...
export const useCurriculumStore = create<CurriculumState>((set, get) => ({
  ...
  loadStandards: async (gradeLevel?: GradeLevel) => {
    ...
    try {
      ...
      set({ error: err instanceof Error ? err.message : 'Failed to load standards', isLoading: false })
    }
  },

  loadReport: async (studentId: string, gradeLevel: GradeLevel, startDate?: string, endDate?: string) => {
    set({ isLoading: true, error: null })
    try {
      const report = await window.api.getCurriculumReport(studentId, gradeLevel, startDate, endDate)
