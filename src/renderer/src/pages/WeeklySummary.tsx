...
export default function WeeklySummary(): JSX.Element {
  ...
  const loadWeekData = useCallback(async () => {
    ...
    try {
      ...
      for (const student of students) {
        const [currentSummary, currentDaily, prevSummary, prevDaily] = await Promise.all([
        ...
        const current: WeekData = {
          ...
          daily: currentDaily,
          totalActivities: currentSummary.reduce((sum, s) => sum + s.totalActivities, 0),
          totalMinutes: currentSummary.reduce((sum, s) => sum + s.totalMinutes, 0),
        ...
        const previous: WeekData = {
          ...
          daily: prevDaily,
          totalActivities: prevSummary.reduce((sum, s) => sum + s.totalActivities, 0),
  ...
  }, [students, currentWeekStart, currentWeekEnd, previousWeekStart, previousWeekEnd])
  ...
  const totals = {
    currentActivities: studentData.reduce((sum, s) => sum + s.current.totalActivities, 0),
    currentMinutes: studentData.reduce((sum, s) => sum + s.current.totalMinutes, 0),
    previousActivities: studentData.reduce((sum, s) => sum + s.previous.totalActivities, 0),
    previousMinutes: studentData.reduce((sum, s) => sum + s.previous.totalMinutes, 0),
  }
    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-2">
          ...
          {weekOffset > 0 && (
            <button onClick={() => setWeekOffset(0)} className="btn btn-primary">
              Current Week
            </button>
          )}
        </div>
      ...
      {isLoading ? (
        <div className="text-center py-12 text-gray-500">Loading weekly summary...</div>
      ) : students.length === 0 ? (
        <div className="card text-center py-12">
          <p className="text-gray-500">No students added yet. Add students in Settings to see weekly summaries.</p>
        </div>
      ) : (
        <>
          {/* Family Overview */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <div className="bg-white/60 rounded-lg p-4 text-center">
                ...
                <div className={`text-sm mt-1 ${getChangeColor(totals.currentMinutes - totals.previousMinutes)}`}>
                  {formatChange(Math.round((totals.currentMinutes - totals.previousMinutes) / 60 * 10) / 10, ' hrs')}
                </div>
              </div>
              <div className="bg-white/60 rounded-lg p-4 text-center">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            {studentData.map(({ student, current, previous, change }) => {
              ...
              return (
                <div key={student.id} className={`card border-l-4 ${colors.border}`}>
                  {/* Student Header */}
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    <div className="text-center p-3 bg-gray-50 rounded-lg">
                      <div className="text-2xl font-bold text-gray-900">{current.totalActivities}</div>
                      <div className="text-xs text-gray-500">Activities</div>
                      <div className={`text-xs mt-1 ${getChangeColor(change.activities)}`}>
                    ...
                    <div className="text-center p-3 bg-gray-50 rounded-lg">
                      ...
                      <div className={`text-xs mt-1 ${getChangeColor(change.minutes)}`}>
                        {formatChange(Math.round((change.minutes / 60) * 10) / 10, 'h')}
                      </div>
                    </div>
                    <div className="text-center p-3 bg-gray-50 rounded-lg">
                      <div className="text-2xl font-bold text-gray-900">{current.activeDays}</div>
                      <div className="text-xs text-gray-500">Days Active</div>
                      <div className={`text-xs mt-1 ${getChangeColor(change.days)}`}>
                  ...
                  <div className="mb-4">
                    <div className="text-xs font-medium text-gray-500 mb-2">Activity by Day</div>
                    <div className="flex gap-1">
                      {weekDays.map((day) => {
                        const dayStr = format(day, 'yyyy-MM-dd')
                        const dayData = current.daily.find((d) => d.date === dayStr)
                        ...
                        return (
                          ...
                          >
                            <div className={`h-8 rounded ${intensityColors[intensity]}`} />
                            <div className="text-xs text-gray-500 mt-1">{format(day, 'EEE')}</div>
                          </div>
                        )
                      })}
                    </div>
                  ...
                  {current.activities.length > 0 && (
                    <div>
                      <div className="text-xs font-medium text-gray-500 mb-2">By Subject</div>
                      <div className="space-y-2">
                        {current.activities.slice(0, 5).map((activity) => {
                          const percentage =
                            current.totalMinutes > 0
                              ? (activity.totalMinutes / current.totalMinutes) * 100
                              : 0
                          return (
                            <div key={activity.subjectId}>
                              <div className="flex justify-between text-xs mb-1">
                                <span className="font-medium text-gray-700">{activity.subjectName}</span>
                                <span className="text-gray-500">
                                  {activity.totalActivities} • {Math.round(activity.totalMinutes)} min
                                </span>
                              </div>
          ...
          <div className="card">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Subject Distribution (All Students)</h2>
            {(() => {
              // Aggregate all subjects across students
              const subjectTotals = new Map<string, { name: string; activities: number; minutes: number }>()
              ...
              const totalMinutes = sortedSubjects.reduce((sum, s) => sum + s.minutes, 0)

              if (sortedSubjects.length === 0) {
                return <p className="text-gray-500 text-sm">No activities recorded this week.</p>
              }
                      ...
                      <div key={subject.name}>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-sm text-gray-500">
                            {subject.activities} activities • {Math.round(subject.minutes / 60 * 10) / 10} hrs (
